...
from app.services.email import send_teacher_credentials_email
...
def create_teacher():
    ...
    if not email_auto_generated:
        # No assignments exist yet at this point (those get added right
        # after via create_teacher_assignment), so this first email will
        # say "no classes/subjects assigned yet" — admin should use the
        # resend-credentials endpoint after assigning classes so the
        # teacher gets the full list.
        send_teacher_credentials_email(teacher, raw_password)
...
@api_bp.route("/teachers/<int:teacher_id>/send-credentials-email", methods=["POST"])
@api_admin_required
def send_teacher_credentials(teacher_id):
    """
    Emails the teacher their login (email + password) and current
    class/subject assignments. Password is pulled from the reversible
    password_encrypted column (see app/services/security.py) so this can
    be triggered any time — right after creation, after adding new
    assignments, or as a manual "resend" from the admin UI.
    """
    teacher = Teacher.query.get_or_404(teacher_id)

    raw_password = decrypt_password(teacher.password_encrypted)
    if not raw_password:
        return jsonify({
            "success": False,
            "message": "No recoverable password on file for this teacher — reset their password first, then resend.",
        }), 400

    sent = send_teacher_credentials_email(teacher, raw_password)
    if not sent:
        return jsonify({
            "success": False,
            "message": "Email could not be sent. Check the teacher has a real email on file and that mail sending is configured.",
        }), 502

    return jsonify({"success": True, "message": f"Credentials sent to {teacher.email}."})
